...
class VAEmodel(BaseModel):
  ...
  def build_model(self):
    ...
    with tf.variable_scope('encoder'):
      ...
      input_tensor = tf.expand_dims(self.original_signal, -1) #마지막 줄에 1을 추가해서 텐서를 확장한다.(차원늘림)
      # print(input_tensor.get_shape())
      # print(input_tensor.shape) #(?,48,1,1)
      # print("11")
      
      # print(input_tensor)
      k_in = tf.pad(input_tensor, [[0, 0], [4, 4], [0, 0], [0, 0]], "SYMMETRIC")
    ...
    with tf.variable_scope('decoder'):
      ...
      print("print_decoded_1:", decoded_1)
      if self.config['l_win'] == 24:
        ...
        decoded_2 = tf.layers.conv2d(decoded_1,
                                     filters=256 * 9,#3을 6으로, 6을 9로
                                     kernel_size=1,
                                     padding='same',
                                     activation=tf.nn.leaky_relu)
        print("printed_decoded_2", decoded_2)
        decoded_2 = tf.reshape(decoded_2, [-1, 3, 3, 256]) #1을 2으로, 2를 3으로
        print("decoded_2 is: {}".format(decoded_2))
        decoded_3 = tf.layers.conv2d(decoded_2,
                                     filters=256,
                                     kernel_size=(3, 1),
        ...
        decoded_3 = tf.nn.depth_to_space(input=decoded_3,
                                         block_size=2)
        decoded_3 = tf.reshape(decoded_3, [-1, 6, 3, 128])  #1을 2로, 
        print("decoded_3 is: {}".format(decoded_3))
        decoded_4 = tf.layers.conv2d(decoded_3,
                                     filters=128,
                                     kernel_size=(3, 1),
        ...
        decoded_4 = tf.nn.depth_to_space(input=decoded_4,
                                         block_size=2)
        decoded_4 = tf.reshape(decoded_4, [-1, 24, 3, 32]) #1을 2로, 2를 3으로
        print("decoded_4 is: {}".format(decoded_4))
        decoded_5 = tf.layers.conv2d(decoded_4,
                                     filters=32,
                                     kernel_size=(3, 1),
        ...
        decoded_5 = tf.nn.depth_to_space(input=decoded_5,
                                         block_size=2)
        decoded_5 = tf.reshape(decoded_5, [-1, 48, 3, 16]) #1을 2로,2 를 3으로
        print("decoded_5 is: {}".format(decoded_5))
        decoded = tf.layers.conv2d(inputs=decoded_5,
                                   filters=1,
...
class lstmKerasModel:
  ...
  def produce_embeddings(self, config, model_vae, data, sess):
    ...
    print(model_vae.original_signal.shape)
    print("data.train_set_lstem",data.train_set_lstm['data'].shape)
